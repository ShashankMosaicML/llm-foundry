...
class CrossEntropyLoss(nn.Module):
    def __init__(
        ...
        """
        super().__init__()
        if reduction not in ["mean", "none", "sum", "weighted_mean"]:
            raise NotImplementedError("Only support reduction = 'mean' or 'none' or 'sum' or 'weighted_mean'")
    ...
    def forward(self, input, target, weights=None):
        ...
        if self.reduction == "mean":
            ...
            return (loss * weights).sum() / ((target != self.ignore_index) * weights).sum()
        else:
            return loss